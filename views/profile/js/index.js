$(function(){
	$("#frmUpdateProfile").on('submit', function(evt){
		...
		$.post(_this.attr('action'), _this.serialize(), function(response){
	        ...
	        function(){
	            window.location = window.siteurl + 'profile';
	        });
		},'json');
	...
	$("#generalModal").on("click", "#okSnapshot", function(evt){
		$("#frmUpdateProfile").find('input[name="image"]').val($("#generalModal").find("#imageSnapshotPreview img").attr("src"));
		$("#frmUpdateProfile").find('.imagePreview img').remove();
		$("#frmUpdateProfile").find('.imagePreview').removeClass('hidden');
		$("#frmUpdateProfile").find('.imagePreview').append("<img style='margin: 0 auto;' class='img-responsive img-bordered' src='"+$("#generalModal").find("#imageSnapshotPreview img").attr("src")+"' />");
	...
	function loadImage(input)
	{
		if (input.files && input.files[0])
		{
			...
			reader.onload = function(e)
			{
				$("#frmUpdateProfile").find('.imagePreview img').remove();
				$("#frmUpdateProfile").find('.imagePreview').removeClass('hidden');
				$("#frmUpdateProfile").find('.imagePreview').append("<img style='margin: 0 auto;' class='img-responsive img-bordered' src='"+e.target.result+"' />");
