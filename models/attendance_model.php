...
class Attendance_Model extends Model
{
    ...
    {
        return $this->db->selectSingleData("SELECT * FROM attendance LEFT JOIN student ON student.studentid = attendance.studentid LEFT JOIN course ON course.courseid = course.courseid WHERE attendance.studentid != $studentId AND attendance.eventid = $eventId AND attendance.semester = ".$this->currentSemester()." AND attendance.schoolyear = '".$this->currentSchoolYear()."' ORDER BY attendance.dateadded DESC");
    }
}