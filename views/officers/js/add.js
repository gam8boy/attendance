$(function(){
	$("#frmAddOfficer").on('submit', function(evt){
		evt.preventDefault();
		...
		$.post(_this.attr('action'), _this.serialize(), function(response){
	        ...
	        function(){
	            $("#frmAddOfficer").find('.imagePreview img').remove();
				$("#frmAddOfficer").find('.imagePreview').removeClass('hidden');
				$("#frmAddOfficer").find('input').val('');
	        });
		},'json');
	...
	$("#generalModal").on("click", "#okSnapshot", function(evt){
		$("#frmAddOfficer").find('input[name="image"]').val($("#generalModal").find("#imageSnapshotPreview img").attr("src"));
		$("#frmAddOfficer").find('.imagePreview img').remove();
		$("#frmAddOfficer").find('.imagePreview').removeClass('hidden');
		$("#frmAddOfficer").find('.imagePreview').append("<img style='margin: 0 auto;' class='img-responsive img-bordered' src='"+$("#generalModal").find("#imageSnapshotPreview img").attr("src")+"' />");
	...
	function loadImage(input)
	{
		if (input.files && input.files[0])
		{
			...
			reader.onload = function(e)
			{
				$("#frmAddOfficer").find('.imagePreview img').remove();
				$("#frmAddOfficer").find('.imagePreview').removeClass('hidden');
				$("#frmAddOfficer").find('.imagePreview').append("<img style='margin: 0 auto;' class='img-responsive img-bordered' src='"+e.target.result+"' />");
				$("#frmAddOfficer").find('input[name="image"]').val(e.target.result);
			}
